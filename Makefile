...
# Copyright (c) 2015-2016 Ivan Balakaev <ivan.uadm at gmail.com>
#
# Version 1.0
#
...
SCRIPTS=mdinit mfsbsd mfsstorage interfaces packages
BOOTMODULES=acpi ahci opensolaris zfs if_tap ciss if_bridge bridgestp
...
xenroothack: install prune xenpackage
${WRKDIR}/.xenroothack_done:
	${_v}if [ -f "${CFGDIR}/menu.rc.local" ]; then \
		${INSTALL} -m 0744 ${CFGDIR}/menu.rc.local ${_DESTDIR}/boot/menu.rc.local; \
	else \
		${INSTALL} -m 0744 ${CFGDIR}/menu.rc.local.sample ${_DESTDIR}/menu.rc.local; \
	fi
	.for FILE in permanent sysctl.conf
...
.endif
	...
	${_v}${INSTALL} ${TOOLSDIR}/create_zpool ${_DESTDIR}/root/bin
...
.for XFILE in menu.rc.local xen.4th xen
	...
	${CP} -rp ${_DESTDIR}/boot/${XFILE} ${WRKDIR}/disk/boot ; \
	else \
	echo -n "No Xen related files ${_BOOTDIR}/${XFILE} "; \
	fi
.endfor
	@echo " done"
